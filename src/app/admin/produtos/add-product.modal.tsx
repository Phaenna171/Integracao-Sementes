...
export default function AddProductModal({ isOpen, onClose }) {
  ...
  async function handleSubmit(e: any) {
    ...
    images.forEach((image, index) => {
      formData.append(`carouselPhotos`, image.file);
    });
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg p-6 w-[700px] max-h-[700px] overflow-y-auto relative">
        ...
        {/* Short Description */}
        <label className="block mb-2">Nome científico:</label>
        <input name="subtitle"
        ...
        {images.map((item, index) => (
          <div className="flex mb-2 gap-4 items-center" key={index}>
            ...
            <label className='border border-green-600 rounded w-1/2 p-2 cursor-pointer' htmlFor={`image-${index}`}>{item.name || 'Selecionar foto'}</label>
            <input id={`image-${index}`} hidden
              accept="image/*"
